...
def get_available_seats(theater_name,selected_date,selected_slots):
	...
	bms_list = frappe.get_list("Book My Show",filters={'theater_name':theater_name,'select_date':selected_date,'select_slots':selected_slots})
...
@frappe.whitelist()
def get_theater(doctype,txt,searchfield,start, page_len,filters):
	theaters = frappe.get_list("Movie Theater",filters={"parent":filters.get('movie_name')},fields=['theater_name'],as_list =1)
	#theater={}
	return [tuple(d) for d in set(theaters)]